  ...
  return (
    <div style={{ padding:'2.25rem' }}>
      ...
      <div style={{ marginTop:'1.5rem', borderTop:'1px solid #f1f5f9', paddingTop:'1.25rem' }}>
        <div style={{ fontSize:'0.7rem', fontWeight:800, color:'#94a3b8', textTransform:'uppercase', letterSpacing:'0.1em', textAlign:'center', marginBottom:'0.75rem' }}>Quick Demo Access</div>
        <div style={{ display:'flex', gap:'0.75rem' }}>
          {demos.map(d => (
            <button key={d.role} onClick={() => setForm({ email:d.email, password:'admin123', remember:false })} style={{ flex:1, padding:'0.55rem', borderRadius:8, background:'#f8fafc', border:'1.5px solid #e2e8f0', cursor:'pointer', fontSize:'0.8rem', fontWeight:700, color:GOV_BLUE, transition:'all 0.2s' }} onMouseEnter={e => e.currentTarget.style.borderColor=ACCENT} onMouseLeave={e => e.currentTarget.style.borderColor='#e2e8f0'}>
              {d.role}
            </button>
          ))}
        </div>
